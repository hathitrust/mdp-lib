...
my $tests = Test::File::load_data("$FindBin::Bin/data/access/ht_total_user.tsv");
...
foreach my $test ( @$tests ) {
    ...
    is(test_attr($attr, $access_profile, $location), $expected_volume, "ht_total_user + attr=$attr + location=$location + profile=$access_profile");
    $num_tests += 1;
}
